...
function createSurfaceFromClosedCurves() {
  ...
  if (boundary2D.length > 0) {
    shape2D.moveTo(boundary2D[0].x, boundary2D[0].y);
    for (let i = 1; i < boundary2D.length; i++) {
      shape2D.lineTo(boundary2D[i].x, boundary2D[i].y);
    }
    shape2D.closePath();
  }
  ...
  boundary2D.forEach(point => {
    centroid2D.add(point);
  });
  centroid2D.divideScalar(boundary2D.length);
  ...
  // Temporary array to store all 3D vertices for distance calculation
  const vertices3D = [];
  ...
  for (let i = 0; i < triangles.length; i++) {
    ...
    for (let j = 0; j < boundary2D.length; j++) {
      const p1 = boundary2D[j];
      const p2 = boundary2D[(j + 1) % boundary2D.length];
    ...
    const distToCentroid = new THREE.Vector2(point2D.x, point2D.y).distanceTo(centroid2D);
  ...
  for (let i = 0; i < frontPositions.length; i += 3) {
    ...
    // Apply non-linear scaling for smoother dome effect (optional)
    // Using a quadratic or cubic function gives a more natural dome shape
    // Square the normalized distance for a quadratic curve
    // const scaleFactor = normalizedDistance * normalizedDistance;
    
    // For a more gradual dome use: normalizedDistance^2 or even higher powers
    const scaleFactor = Math.pow(normalizedDistance, 2);
