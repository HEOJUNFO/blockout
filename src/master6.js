...
function createSurfaceFromClosedCurves() {
  ...
  // 1. Collect all boundary points from curves with deduplication to avoid overlaps
  const boundaryPoints = [];
  const pointsSet = new Set(); // For deduplication
  ...
  curveLines.forEach(curveLine => {
    ...
    for (let i = 0; i < positions.count; i++) {
      ...
      // Deduplicate points that are very close (helps avoid cracks)
  ...
  // 2. Calculate average normal from the clicked points (more robust)
  const averageNormal = new THREE.Vector3();
  clickPoints.forEach(pointData => {
    averageNormal.add(pointData.normal);
  });
  
  // Ensure a valid normal
  if (averageNormal.length() < 0.001) {
    // Fallback to Z-up if the normal is too small
  ...
  // 4. Project the boundary points onto the plane with more robust axes
  ...
  // Project boundary points to 2D with improved accuracy
  const boundary2D = [];
  boundaryPoints.forEach(point => {
  ...
  for (let i = 0; i < triangles.length; i++) {
    ...
    if (vertexMap.has(key)) {
      ...
      // Multiple raycasts with different offsets to improve reliability
      ...
      for (const offset of raycastOffsets) {
        ...
        raycaster.firstHitOnly = true; // Use this option for better performance
  ...
  // Find the maximum possible distance from the border (approximation of the "center")
  // Get the centroid of the 2D shape
  ...
  // Calculate approximate max distance (from centroid to furthest boundary point)
  ...
  // 7. Create back face positions by offsetting front face along normals with variable extrusion
  const backPositions = new Float32Array(frontPositions.length);
  const backNormals = new Float32Array(frontNormals.length);
  
  // Calculate the distance from border for each vertex
  const distancesFromBorder = [];
  
  for (let i = 0; i < triangles.length; i++) {
    const point2D = triangles[i];
    
    // Calculate distance to the nearest border point
    ...
    // Normalize the distance (0 at border, 1 at maximum distance from border)
    // Use a formula that gives distance from border
  ...
  // Filter out invalid triangles by checking for NaN or invalid coordinates
  ...
  for (let i = 0; i < triangles.length / 3; i++) {
    ...
    // Skip triangle if any vertex has NaN coordinates or is too close to another vertex
  ...
  // Apply the extrusion based on distance from border
  ...
  for (let i = 0; i < frontFaceCount; i++) {
    ...
    // Skip if any vertex has invalid coordinates
    let hasInvalidCoord = false;
    for (let j = 0; j < 3; j++) {
  ...
  for (const [key, [a, b]] of edgeMap.entries()) {
    ...
    // Skip if any vertices have invalid coordinates
    ...
    if (isNaN(wallNormal.x) || isNaN(wallNormal.y) || isNaN(wallNormal.z) ||
      ...
      // Use average normal as fallback
